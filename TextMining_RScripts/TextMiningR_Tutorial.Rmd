...
1.  Create the corpus (In the data provided I have done the cleaning/pre-processing of the free-text field already)

2.  Build the DFM

3.  Split the data into a training and a testing dataset
...
dim(coded_freetext)
head(coded_freetext)
summary(coded_freetext)
  labels<-table(coded_freetext$label)
  meanings<-table(coded_freetext$meaning)
  (better_summary<-data.frame(meanings) %>% mutate(labels = names(labels)))

#*************************************************************
# 4. Create the corpus ====
freetext_corpus<-corpus(coded_freetext[,1:3 ], text_field = 'freetext_clean')
#see bits of the corpus:
texts(freetext_corpus)[9:11]

#add the doc_id as to docvars
#docvars(freetext_corpus, field = 'doc_id') <- coded_freetext$doc_id
...
svm_predicted_class <- predict(svm_model, newdata = testing_data, type = "probability", force = T)
...
#remove low freq words and terms only found in a few documents
newfields_dfm_nolows <-dfm_trim(newdata_dfm, min_termfreq = 5, min_docfreq = 3)

#Match the features of the new corpus to that used to build the model
newfields_matched <- newfields_dfm_nolows
