...
#Can be a really good idea to remove very sparse terms from the dfm
(dfmat_covidtweets<-dfmat_covidtweets %>% dfm_trim(min_termfreq = 10, verbose = FALSE))
...
We will apply the the Lexicoder Sentiment Dictionary to the selected contexts using **textstat_polarity()**. All three of these lexicons are based on unigrams (i.e., single words), and bigrams. These lexicons contain many English words and the words are assigned scores for positive/negative sentiment, and also possibly emotions like joy, anger, sadness, and so forth.
...
##default function == log(\frac{pos}{neg}) --> If you have very long documents - you may want to weight the sentiment values by the length (total words) in the document. There are other functions that can do this for you - or you can write your own and feed it in using the option 'fun = xxx'
...
# This script:
#1. load the freetext field corpus
#1. Splits the dataset into training and testing sets
#2. build a sparse dfm matrix
#3. runs a XXX model/s using the sparse matrix as input
#4. predicts with the model to the test data
...
coded_freetext<-fread(here('Data/RFS_recoded_freetextfields.txt'))
...
mystopwords<-c('participant', 'patient', 'p', 'p\'s', 'p-', 'ps', 'p:', 'p', 'pt', 'ppt', 'p.', 'p,', 'opt', 
               'rt', 'right', 'rhs', 'lhs', 'lt', 'left', 'authorised', 'DM', 'dm', 'tp', 're', 'ch', 'ca', 'oh', 'ohs', 'ot',
               'mr', 'mrs', 'miss', 'participanrt', 'x2', '2x', '3x', 'x3')
