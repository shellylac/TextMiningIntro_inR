...
coded_freetext<-read_tsv(here('Working_Data/Supervised_ML/labelled_dataset.txt')) %>% arrange(doc_id)
dim(coded_freetext)
head(coded_freetext)
...
dfm_nolowfreqs <-dfm_trim(dfm, min_termfreq = 6, min_docfreq = 3)
dfm_nolowfreqs
  dim(dfm_nolowfreqs)
...
set.seed(4321)
training_data <- dfm_sample(dfm, size = 0.6*nrow(dfm))
testing_data<-dfm[setdiff(docnames(dfm), docnames(training_data)), ]
```
